...
function togglePropertyList(propertyName){
	let ul = $("#" + propertyName).next();
	let image = $("#" + propertyName + " > .toggleImage");
	if (!ul.is(":hidden")){
		image.attr('src', image.attr('src').replace("collapse.svg", "expand.svg"));		
		ul.hide(400);
	} else {
		image.attr('src', image.attr('src').replace("expand.svg", "collapse.svg"));
		ul.show(400);
	}		
